...
export default function FormateursPage() {
  ...
  return <div className="mx-auto max-w-[1440px] space-y-5 pb-10">
    <div className="flex flex-wrap items-end justify-between gap-4 border-b border-slate-200 pb-5">
      <div><p className="text-xs font-semibold uppercase text-emerald-700">Espace formateurs</p><h1 className="mt-1 text-2xl font-semibold text-slate-950">Équipe & planning</h1></div>
    ...
    {tab === "team" ? <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
      ...
      {formation && <>
        <div className="border-y border-slate-200 py-3"><div className="mb-2 flex items-center justify-between"><h2 className="text-sm font-semibold">Formateurs de la session</h2><button type="button" onClick={() => setTab("team")} className="text-xs text-emerald-700">Gérer l&apos;équipe</button></div><div className="flex flex-wrap gap-2">{trainers.map((trainer) => <button key={trainer.id} type="button" disabled={busy} onClick={() => void toggleTrainer(trainer.id)} aria-pressed={formation.trainerIds?.includes(trainer.id) || false} className={`rounded-full border px-3 py-1.5 text-sm ${formation.trainerIds?.includes(trainer.id) ? "border-emerald-700 bg-emerald-50 text-emerald-900" : "border-slate-200 bg-white text-slate-600"}`}>{formation.trainerIds?.includes(trainer.id) && <Check size={13} className="mr-1 inline" />}{trainerName(trainer)}</button>)}{!trainers.length && <p className="text-sm text-slate-500">Ajoute d&apos;abord un formateur dans l&apos;onglet Équipe.</p>}</div></div>
